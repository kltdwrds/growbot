...
function onSkuDetails(response) {
    ...
    while (productsTBody.firstChild) {
        productsTBody.removeChild(productsTBody.firstChild);
    }
...
function onSkuDetailsFailed(response) {
    console.log("onSkuDetailsFailed");
    console.log(response.response.errorType);
    statusDiv.text("Error retrieving product list. (" + response.response.errorType + ")");
    if (response.response.errorType == 'TOKEN_MISSING_ERROR') {
        $('.notLoggedIn, #productsTBody button').show().click(function() { chrome.extension.sendMessage({ "fnc": "getLicenseInteractive" }); });
    }
}
...
function addProductToUI(product) {
    // DON'T OFFER THIS SUBSCRIPTION ANYMORE 
    if (product.sku == 'growbot_monthly_subscription') return false;
